\title{Monolingual Post-Editing: An Exploratory Study on Research Behaviour and Target Text Quality}
...
\chapter{Data Set Description and Methodology}
...
\chapter{Quality of the Monolingual Post-Edited Texts}
...
\multicolumn{3}{l}{Major content mistakes}\\
...
\lspbottomrule
\end{tabular}
\caption{Examples of Content Mistakes through MPE}
\label{tab:1}
\end{table}
...
\section{Interpretation of Error Analysis}
...
\chapter[Research Behaviour]{Research Behaviour}
...
\begin{figure}[h]
 \includegraphics[width=\textwidth]{Nitzke1.png}
...
\begin{table}
\begin{tabular}{lllllll} & Text 1 & Text 2 & Text 3 & Text 4 & Text 5 & Text 6\\
\lsptoprule
...
\begin{quotation}
  ...
  \textbf{GazeT}: Total gaze time on target text unit […]” (ibid.: 22) \end{quotation}
...
phrases & Total amount of Sessions

(MPE sessions) & Total amount of sessions with research on the word/

phrase & Total amount of sessions with no research & Total amount of sessions with research in MPE (n) & Total amount of sessions with no research in MPE\\
...
The mean values for Dur (Table \ref{tab:4}) and GazeT (Table \ref{tab:5}) will be compared for the whole dataset with the MPE data in the next paragraphs. No statistical evaluation will be conducted for the data, because n is very small for the research instances per word/phrase task (see Table \ref{tab:3}). In a follow-up study the number of participants would have to be increased to find statistically significant differences between the different tasks. It is expected that \textit{Dur }and\textit{ GazeT }are higher for researched words/phrases in MPE than the other tasks. The parameter should be approximately the same when the words/phrases were not researched.
...
Dur & below-inflation & cut interest rates & rattle & halt & Khartoum & incentives & associate & exposure & bureaucrats & full-time leader\\
...
GazeT & below-inflation & cut interest rates & rattle & halt & Khartoum & incentives & associate & exposure & bureaucrats & full-time leader\\
